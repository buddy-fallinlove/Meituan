...
export default {
    ...
    // city=城市名
    getCityInfo() {
        return service.req(`/crumbs?city=${city}`)
    },
    //省市关联(5)
    association() {
    ...
    // city=城市名
    addCart() {
        return service.req(`/hotPlace?city=${city}`)
    },
    // 搜索建议(9)
    // city=城市名  input=关键词
    searchTop(value, keyword) {
        return service.req(`/searchTop?city=${value}&input=${keyword}`)
    },
    //根据关键词搜索(10)
    // city=城市名  keyword=关键词
    keySearch() {
        return service.req(`/results?city=${city}&keyword=${keyWord}`)
    },
    // 商铺详情(11)
